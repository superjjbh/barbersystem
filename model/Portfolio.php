...
class Portfolio extends Controle {
    ...
    public function atualizar() {
        if ($this->portfolio_imagem != "") {
            $this->update("portfolio", "portfolio_nome = '$this->portfolio_nome', portfolio_cliente = '$this->portfolio_cliente', portfolio_descricao = '$this->portfolio_descricao', portfolio_url = '$this->portfolio_url', portfolio_imagem = '$this->portfolio_imagem', portfolio_area1 = '$this->portfolio_area1'"
                    . ",portfolio_imagem = '$this->portfolio_imagem'", "portfolio_id = '$this->portfolio_id'");
        } else {
            $this->update("portfolio", "portfolio_nome = '$this->portfolio_nome', portfolio_cliente = '$this->portfolio_cliente', portfolio_descricao = '$this->portfolio_descricao', portfolio_url = '$this->portfolio_url', portfolio_area1 = '$this->portfolio_area1'", "portfolio_id = '$this->portfolio_id'");
        }
    }
    ...
    public function getPortfolios() {
        $this->select("portfolio", "", "*", "portfolio_nome ASC", "INNER JOIN area1 ON (portfolio_area1 = area1_id)", "");
    }
	
    public function getPortfoliosUltimos() {
        $this->select("portfolio", "", "*", "portfolio_id DESC", "INNER JOIN area1 ON (portfolio_area1 = area1_id)", "");
    }
	
    public function getSearch() {
        $busca = $_POST['busca'];
        $this->db->query("SELECT * FROM portfolio INNER JOIN area1 as a ON (portfolio_area1 = area1_id)WHERE portfolio_nome like '%$busca%' or portfolio_cliente like '%$busca%' or a.area1_nome LIKE '%$busca%'")->fetchAll(); //OR 
        $this->db->data;
    }
    ...
    public function getUltimosSide() {
        $this->select("portfolio", "", "*", "", "ORDER BY portfolio_id DESC LIMIT 0,8", "");
    }
    /* PORTFOLIO ULTIMOS 6 RODAPE*/
