...
class Pagina extends Controle {
    ...
    public function getPaginasAtiva() {
        $this->select("pagina", "", "*", "", "INNER JOIN area ON (pagina_area = area_id) WHERE pagina_data = 'SIM' ORDER BY pagina_area ASC", "");
    }
	
    public function getPaginasAss() {
        $this->select("pagina", "", "*", "", "INNER JOIN area as a ON (pagina_area = area_id) WHERE a.area_nome = 'Assessoria' ORDER BY pagina_nome ASC", "");
    }
    ...
    public function getPaginasOutros() {
        $this->select("pagina", "", "*", "", "INNER JOIN area as a ON (pagina_area = area_id) WHERE a.area_nome <> 'Assessoria' ORDER BY pagina_nome ASC", "");
    }
	
    public function getBusca() {
		$busca = $_POST['busca'];
        $this->db->query("SELECT * FROM pagina as p INNER JOIN area as a ON (pagina_area = area_id) WHERE a.area_nome like '%$busca%'")->fetchAll(); //OR 
        $this->db->data;
    }
	
    public function getServicoAdicional() {
        $busca = $pagina->pagina_nome;
		$this->db->query("SELECT * FROM pagina WHERE pagina_nome <> '$busca'")->fetchAll(); //OR 
        $this->db->data;
    }
	
    public function getAdicionais($pagina_id) {
		$this->select("pagina", "", "*", "", " WHERE pagina_id <> $pagina_id ORDER BY pagina_nome", "");
    }
