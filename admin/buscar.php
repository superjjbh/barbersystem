...
//Pesquisa servicos
$resultBusca = mysqli_query($conn, "SELECT * FROM servico where servico_nome like '%$busca%' ORDER BY servico_data DESC") or die(mysql_error());


$resultStatus = mysqli_query($conn, "SELECT * FROM servico where servico_status = $status");
$buscaStatus = mysqli_fetch_assoc($resultStatus);

$resultPeriodo = mysqli_query($conn, "SELECT * FROM servico WHERE servico_data BETWEEN '$inicio' AND '$fim' ORDER BY servico_data DESC");
$buscaPeriodo = mysqli_fetch_assoc($resultPeriodo);
...
<!--[if !IE]><!--> <html lang="en"> <!--<![endif]-->
    ...
    <body>
		...
		<?php if (isset($_POST['busca'])) : ?>
						<?php
						while($service = $buscaCliente = mysqli_fetch_assoc($resultBusca);) { ?>
								
						<div class="panel-group" id="accordion">
							<div class="panel panel-default">
								<div class="panel-heading">
									<h4 class="panel-title">
										<a class="accordion-toggle collapsed" data-toggle="collapse" data-parent="#accordion" href="#<?php echo $service['servico_id']; ?>">
											
													<?php if (echo $service['servico_status'] == 1): ?>
													<img src="./images/confirma.png" alt="CONFIRMADO" />  Data: <?= echo $service['servico_data']; ?> </br> Cliente: <?= echo $service['servico_nome']; ?> | Horário: <?= echo $service['servico_horario']; ?>
													<?php endif; ?>
													<?php if (echo $service['servico_status'] == 3): ?>
													<img src="./images/pago.png" alt="PAGO E CONFIRMADO" />  Data: <?= echo $service['servico_data']; ?> </br> Cliente: <?= echo $service['servico_nome']; ?> | Horário: <?= echo $service['servico_horario']; ?>
													<?php endif; ?>
													<?php if (echo $service['servico_status'] == 2): ?>
													<img src="./images/atendido.png" alt="ATENDIDO" />  Data: <?= echo $service['servico_data']; ?> </br> Cliente: <?= echo $service['servico_nome']; ?> | Horário: <?= echo $service['servico_horario']; ?>
													<?php endif; ?>
													<?php if (echo $service['servico_status'] == 0): ?>
													<img src="./images/aguardo.png" alt="PENDENTE" />  Data: <?= echo $service['servico_data']; ?> </br> Cliente: <?= echo $service['servico_nome']; ?> | Horário: <?= echo $service['servico_horario']; ?>
													<?php endif; ?>
										</a>
									</h4>
								</div>
								<div id="<?php echo $service["servico_id"]; ?>" class="panel-collapse collapse" style="height: 0px;">
									<div class="panel-body">
										<h3>Detalhes do agendamento:</h3>
										<b>Nome:</b> <?= echo $service['servico_nome']; ?></br>
										<b>Contato:</b> <?= echo $service['servico_icon']; ?></br>
										<b>E-mail:</b> <?= echo $service['servico_email']; ?></br>
										<hr>
										<b>Unidade:</b> <?= echo $service['servico_unidade']; ?></br>
										<?php if (empty($service->servico_promocao)) : ?>
										<b>Serviço Solicitado:</b> Nº: <?= $service->servico_id ?> - <?= echo $service['servico_descricao']; ?></br>
										<?php endif; ?>
										<?php if (!empty($service->servico_promocao)) : ?>
										<b>Promoção Escolhida:</b> <?= echo $service['servico_promocao']; ?></br>
										<?php endif; ?>
										...
										<b>Data Escolhida:</b> <?= echo $service['servico_data']; ?></br>
										<b>Horário Escolhido:</b> <?= echo $service['servico_horario']; ?></br>
										...
										<?php if ((echo $service['servico_status'] == 1) && (stripslashes($service->servico_pagamento) == 'Direto na Barbearia')): ?>
										...
										<?php endif; ?>
										<?php if ((echo $service['servico_status'] == 1) && (stripslashes($service->servico_pagamento) == 'Online (Pagseguro)')): ?>
