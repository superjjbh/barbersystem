                        ...
                        <div class="col-md-12">
                            <div class="panel rounded shadow">
                                <div class="panel-sub-heading">
                                    <div class="callout callout-info"><p><strong>Editar Destaque</strong></p> <i class="fa fa-exclamation-triangle"></i> Descrição no máximo 150 caracteres </div>
                                </div>
                                <div class="clearfix"></div>
                                <div class="panel-body no-padding">
							    <?php
                                    if (isset($_POST['servico_nome']) && !empty($_POST['servico_nome'])) {
                                        
										date_default_timezone_set('America/Sao_Paulo');
									  //envio o charset para evitar problemas com acentos
									  header("Content-Type: text/html; charset=UTF-8");
									  $mysqli = new mysqli('localhost', 'root', '', 'seguranca');
									  $horario = filter_input(INPUT_POST, 'servico_horario');
									  $servico = filter_input(INPUT_POST, 'servico_descricao');
									  $status = 1;
									  $sql = "SELECT * FROM `servico` WHERE `servico_horario` = '{$horario}' AND `servico_descricao` = '{$servico}' AND `servico_status` = '{$status}'"; //monto a query
									  $query = $mysqli->query( $sql ); //executo a query
									  
										  if( $query->num_rows > 0 ) {//se retornar algum resultado
										echo '<div class="alert alert-danger" > Desculpe! Mas já existe uma confirmação de agendamento nessa data e horário! Escolha outro horário ou data para verificarmos a disponibilidade!</b></br></div>';
									...
									<div class="form-body">
										...
										<form method="post" action="servico_fn.php?acao=atualizar">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input class="form-control rounded" type="text" id="servico_icon" name="servico_icon" required value="<?= stripslashes($servico->servico_icon) ?>" />
                                            </div>
											
											<div class="form-group">
                                                <label class="control-label">Horário</label>
                                                <input class="form-control rounded" type="text" id="servico_horario" name="servico_horario" required value="<?= stripslashes($servico->servico_horario) ?>" />
                                            </div>
